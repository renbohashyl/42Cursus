...
ClapTrap::ClapTrap(std::string setName, unsigned int setHP, unsigned int setMP, unsigned int setDMG)
	: name(setName), hp(setHP), maxhp(setHP), mp(setMP), dmg(setDMG) {
	std::cout << "ClapTrap : " << this->name << " : Hi! Vault Hunter!" << std::endl;
}
...
ClapTrap& ClapTrap::operator=(ClapTrap const& copy_assign) {
	std::cout << "after this message, " << this->name;
	if (this != &copy_assign) {
	...
	std::cout << " will be copy production of " << copy_assign.name << std::endl;
	return (*this);
}
